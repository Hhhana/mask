...
def processing_data(data_path, height=224, width=224, batch_size=32,
    ...
    valid_data_loader = DataLoader(test_dataset, batch_size=batch_size,shuffle=True) # 加载验证/测试数据

    return train_data_loader, valid_data_loader
# 定义完函数后开始调用
# mask共313张，nomask共443张，数据集一共756张
data_path = './datasets/5f680a696ec9b83bb0037081-momodel/data/image' # 向上一层获取当前目录
...
for index, (x, labels) in enumerate(train_data_loader):
    print(index, "\nfeature:",x[0], "\nlabels:",labels) # 打印每个batch中第一张图片的feature，和所有label
    show_tensor_img(x)
...
for epoch in range(epochs):
    ...
    for batch_idx, (x, y) in tqdm(enumerate(train_data_loader, 1)):# 批处理
        x = x.to(device)
        y = y.to(device)
        pred_y = model(x)
